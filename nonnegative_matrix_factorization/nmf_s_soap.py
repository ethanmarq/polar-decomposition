...
class NMFModel(nn.Module):
    ...
    def forward(self, target):
        # Enforce nonnegativity in forward pass
...
def main(seed=42, steps=1000):
    ...
    def train_lowrank(optimizer_cls, method='qdwh', lr=0.1, steps=steps, scheduler=False):
        ...
        if optimizer_cls == torch.optim.Adam:
            ...
            optimizer = optimizer_cls(model.parameters(), lr=lr, weight_decay=0., precondition_frequency=1)
        else:
    ...
    loss_soap, cond_X_soap, cond_Y_soap, nuc_X_soap, nuc_Y_soap = train_lowrank(SOAP, lr=3e-1)
    ...
    axes[1].plot(cond_X_soap, linestyle=':')
    ...
    axes[2].plot(cond_Y_soap, linestyle=':')
    ...
    fig.legend(loc='outside lower center', ncol=9, bbox_to_anchor=(0.5, -0.05), borderaxespad=0., fontsize=16)
