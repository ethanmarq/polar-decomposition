...
# Objective: minimize 0.5 * ||A X - C||_F^2
def loss_fn(X, A, C):
    return 0.5 * torch.linalg.matrix_norm(A @ X - C) ** 2

# Model: learnable matrix X
class MultiResponseLinearRegressionModel(nn.Module):
    def __init__(self, m=500, n=100):
        super().__init__()
        self.X = nn.Parameter(torch.empty(m, n).uniform_(-1., 1.))

    def forward(self, A, C):
        return loss_fn(self.X, A, C)
...
def inverse_hessian_preconditioner(A):
    AtA = A.T @ A  # m x m
    AtA_inv = torch.linalg.inv(AtA)
    return AtA_inv

def inverse_hessian_preconditioner_2(A):
    AtA = A.T @ A
    AtA_inv = torch.linalg.inv(AtA)
    return AtA_inv, AtA
...
def main(seed=42, steps=4000):
    ...
    hessian_cond_number = (torch.linalg.cond(A)**2).item()
    print(f"Hessian condition number: {hessian_cond_number}")
    eigvals_A = torch.linalg.eigvalsh(A.T @ A)
    print(f"Hessian condition number: {eigvals_A[-1] / eigvals_A[0]}")

    AtA_inv = torch.linalg.inv(A.T @ A)     # (m x m)
    X_star = AtA_inv @ A.T @ C
    print(f"Loss at X_star: {loss_fn(X_star, A, C).item()}")

    # Optimization loop
    def run_quadratic(optimizer_cls, method='qdwh', steps=steps, lr=0.05, scheduler=False):
        torch.manual_seed(seed)
        model = MultiResponseLinearRegressionModel(m, n).to(device)
        ...
        for _ in tqdm(range(steps), desc=f"optimizer = {optimizer_cls.__name__}, polar decomp method = {method if optimizer_cls != torch.optim.Adam else None}, lr decay = {scheduler if isinstance(scheduler, bool) else scheduler.__class__.__name__}"):
            ...
            if scheduler:
                scheduler.step()
            losses.append(loss.item() - loss_fn(X_star, A, C).item())
            condition_numbers_residual.append(torch.linalg.cond(A @ model.X - C).item())
            condition_numbers_grad.append(torch.linalg.cond(grad(model.X, A, C)).item())
            nuc_norms_grad.append(torch.linalg.matrix_norm(grad(model.X, A, C), ord='nuc').item())
    ...
    def run_inverse_hessian(steps=steps, lr=0.05):
        torch.manual_seed(seed)
        model = MultiResponseLinearRegressionModel(m, n).to(device)
        AtA_inv = inverse_hessian_preconditioner(A)
        losses = []
        condition_numbers_residual = []
        condition_numbers_grad = []
        nuc_norms_grad = []
        for _ in tqdm(range(steps), desc=f"optimizer = Newton"):
            X = model.X
            G = grad(X, A, C)
            precond_grad = AtA_inv @ G
            model.X.data -= lr * precond_grad
            losses.append(loss_fn(model.X, A, C).item() - loss_fn(X_star, A, C).item())
            condition_numbers_residual.append(torch.linalg.cond(A @ model.X - C).item())
            condition_numbers_grad.append(torch.linalg.cond(grad(model.X, A, C)).item())
            nuc_norms_grad.append(torch.linalg.matrix_norm(grad(model.X, A, C), ord='nuc').item())
        condition_numbers_residual = smooth(condition_numbers_residual)
    ...
    axes[1].set_ylabel(r"$\kappa_2(A X_k - C)$")
    ...
    fig.savefig(f'fig/multi_lin_reg_{seed}.pdf', dpi=500, bbox_inches='tight')
    plt.close(fig)
    ...
    fig2.savefig(f'fig/multi_lin_reg_nuc_{seed}.pdf', dpi=500, bbox_inches='tight')
    plt.close(fig2)
